...
/**
 * get_pid - Get the current process ID.
...
/**
 * get_env_value - value corresponding to an environmental variable.
 ...
 *         Else - value of environmental variable.
 *
...
{
	int j, k = 0, len;
	char *replacer = NULL, *old_line = NULL, *new_line;
	...
	for (j = 0; old_line[j]; j++)
	{
				...
				old_line[j + 1] != ' ')
		{
			...
			{
				replacer = _itoa(*exe_ret);
				k = j + 2;
			}
			else if (old_line[j + 1])
			{
				/* Extraction of variable name to search FOR */
				for (k = j + 1; old_line[k] &&
						old_line[k] != '$' &&
						old_line[k] != ' '; k++)
					;
				len = k - (j + 1);
				replacer = get_env_value(&old_line[j + 1], len);
			}
			new_line = malloc(j + _strlen(replacer)
